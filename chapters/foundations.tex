...
where:
\begin{itemize}
  \item \( f_0 \in \mathbb{R} \) is a constant,
  \item \( f_1(x_1) \), \( f_2(x_2) \), and \( f_{1,2}(x_1, x_2) \) have zero mean in their own variables,
  \item all components are mutually orthogonal in \( L^2([0,1]^2) \).
\end{itemize}

\vspace{1em}

\textbf{Step 1: Square the decomposition.}
...
\textbf{Step 2: Integrate over the domain \( [0,1]^2 \).}
...
\textbf{Step 3: Define the variance.}

\[
D := \int f(x_1, x_2)^2 \, dx_1 dx_2 - f_0^2
\]

Then the total variance decomposes as:
