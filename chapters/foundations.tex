...
\begin{align*}
    ...
    &= \mathbb{E}[y(\boldsymbol{X}) \mid X_i = x_i)] - y_0 = y_i(x_i)
\end{align*}
...
When i start from \cite{muehlenstaedt2012} and basically go the way \textit{fANOVA term $\rightarrow$ expected value $\rightarrow$ projection}, I arrive at the formulation where we first compute the projection and afterwards subtract lower order terms.
But when I get it correctly \cite{hooker2007} actually writes his generalized fANOVA components actually as the projection of the differences, so he basically goes the other way around, starting from the projections (and we could restate this as the conditional expected value).
...
\end{equation}
And I think we can rewrite this as the conditional expected value. We write an example for $u = \{1,2\}$:
\begin{align*}
    y_{12}(x_1, x_2) &= \arg \min_{g_{12} \in L^2(\mathbb{R}^2)} \int \left( y(\boldsymbol{X}) - \sum_{\{1,2\}} g_u(x_u) \right)^2 w(\boldsymbol{x}) \, d\boldsymbol{x} \\
    &= \arg \min_{g_{12} \in L^2(\mathbb{R}^2)} \int \left( y(\boldsymbol{X}) - y_\emptyset - y_1 - y_2 - g_{12}(x_1, x_2) \right)^2 w(\boldsymbol{x}) \, d\boldsymbol{x} \\
    &= \arg \min_{g_{12} \in L^2(\mathbb{R}^2)} \mathbb{E}[(y(\boldsymbol{X}) - y_\emptyset - y_1 - y_2 - g_{12}(x_1, x_2))^2] \\
    &= \mathbb{E}[y(\boldsymbol{X}) - y_{\emptyset} - y_1(x_1) - y_2(x_2) | X_1 = x_1, X_2 = x_2]
\end{align*}
So he takes the projection of the difference. 
...
\begin{align*}
    \Pi_{\mathcal{G}_i}(y - y_0)
    &= \arg \min_{g \in \mathcal{G}_i} \|y - y_0 - g_i\|^2\\
    &= \arg \min_{g \in \mathcal{G}_i} \mathbb{E}[((y(\boldsymbol{X}) - y_0) - g(X_i))^2] \\
    &= \mathbb{E}[(y(\boldsymbol{X}) - y_0) \mid y(X_1) = y_(x_1)] \\
    &= \mathbb{E}[(y(\boldsymbol{X})) \mid y(X_1) = y_(x_1)] - y_0 \text{ if $y_0$ is regarded as a constant term}
\end{align*}
