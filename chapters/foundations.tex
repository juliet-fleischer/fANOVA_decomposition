...
Referring to the general connection between the expected value and orthogonal projections presented in section~\ref{background}, the fANOVA terms can also be understood from a viewpoint of projections. This will also help to understand the generalization of fANOVA in section~\ref{generalization}.\par

$y_\emptyset$ is the projections of the original function $y$ onto the space of all constant functions $\mathcal{G}_0 = \{g(x) = a; a \in \mathbb{R}\}$. It is an unconditional expected value and the best approximation of $y$ given a constant function:
\begin{align*}
    \Pi_{\mathcal{G}_0}y
    &= \arg \min_{g \in \mathcal{G}_0} \|y(\boldsymbol{X}) - g\|^2 \\
    &= \arg \min_{g \in \mathcal{G}_0} \mathbb{E}[(y(\boldsymbol{X}) - g)^2] \\
