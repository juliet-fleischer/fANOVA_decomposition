...
\subsection{Alternative Definition of Generalized fANOVA Components}
...
It is not feasible to enforce the hierarchical orthogonality conditions as written above {\color{blue} because?? or is this even the ``reason'' for Lemma 1}. 
Lemma 1 provides the foundation for Hookers generalization because it gives a tangible constraint, one can set to ensure that one really obtains fANOVA components (which satisfy hierarchical orthogonality) instead of the components of the Hoeffding decomposition we saw in our example.
\begin{lemma}
...
\tag{4.3}
\end{equation}
\label{lemma:hooker_hierarchical_orthogonality}
\end{lemma}
...
As we saw earlier, the expected value of the generalizes fANOVA decomposition is the constant term:
\begin{equation}
